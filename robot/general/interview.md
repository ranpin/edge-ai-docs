...
两者都是 Linux 实时化方案，用于满足机器人控制的硬实时需求。RT-PREEMPT（PREEMPT\_RT 补丁）将 Linux 内核改造为完全可抢占的，关键修改包括：将 spinlock 替换为可睡眠的 rt\_mutex、将中断处理线程化（threaded IRQ）、引入优先级继承解决优先级反转。其优势是与标准 Linux 完全兼容，应用开发无需修改，生态丰富（ROS2 官方支持），典型最坏延迟在 50-100 微秒级别。Xenomai 采用双内核架构（Cobalt 实时微内核 + Linux 内核），实时任务运行在 Cobalt 上优先级高于所有 Linux 任务，典型最坏延迟可达 10-30 微秒。但 Xenomai 需要使用专用 API（Alchemy/POSIX skin）开发实时任务，与标准 Linux 应用不完全兼容，驱动也需要适配，开发门槛较高。选择建议：如果控制频率在 1kHz 以下（如具身操作视觉伺服、低频感知/规划），RT-PREEMPT 足够且开发更简便；如果需要 1kHz 以上的控制频率（如力控机械臂、双足平衡、灵巧手），或对延迟抖动要求极严格（< 20us），选择 Xenomai。近年来 RT-PREEMPT 已被合并进 Linux 主线内核（6.x），成熟度大幅提升，在大多数机器人场景中已成为首选。
...
具身/人形机器人需要记忆用户偏好、环境变化、物体位置和历史操作经验，长期记忆系统是实现个性化与长程任务的基础。记忆分层架构：参考人类记忆模型设计三层——工作记忆（当前对话上下文 + 实时感知信息，存储在内存中，容量受限，会话结束后通过摘要写入长期记忆）；情景记忆（具体事件记录，如 "2024-01-15 用户张三要求把水杯放到餐桌左侧"，以结构化 JSON + 向量嵌入存储在 SQLite + 向量数据库中）；语义记忆（抽象化的知识和偏好，如 "用户张三偏好水杯放在左手边"、"厨房的杯子通常在上层橱柜"，由多次情景记忆归纳提炼而来）。

具身记忆的特殊性在于**空间接地**：记忆不只是文本，还要绑定 3D 空间——物体级语义地图（见 [部署篇](deployment.md) §5.2 语义 SLAM）是情景记忆的空间载体，"杯子在桌上"这类记忆需要与物体位姿关联，才能支撑"去拿那个杯子"的长程任务。记忆写入：每次交互/操作结束后，使用端侧 LLM 从对话与执行轨迹中提取关键信息（人物、事件、偏好、物体位姿、成功/失败），生成情景记忆条目并向量化存储；定期（如每天夜间）对累积的情景记忆进行归纳总结，更新语义记忆。记忆检索：当新任务发生时，根据当前上下文（用户身份、场景、指令）从记忆库中检索相关记忆——向量相似度检索 + 时间衰减加权 + 元数据过滤（如只检索该用户/该区域的记忆），将检索到的记忆注入 LLM prompt 的上下文中。记忆管理：实现遗忘机制防止记忆无限增长——基于访问频率和时间衰减的重要性评分，定期清理低重要性的情景记忆；隐私保护方面，进入家庭的人形采集的敏感信息（居家环境、人脸、健康）加密存储，用户可随时要求删除其所有记忆数据。端侧实现：SQLite 存储结构化记忆（<100MB），Hnswlib 存储向量索引（<50MB），总计端侧存储开销 <200MB，完全可以在人形端侧算力上运行。

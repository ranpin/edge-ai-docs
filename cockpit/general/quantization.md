...
| **W4A16（AIMET/QAIRT · activation-aware 校准）** | INT4 权重 / FP16 激活 | ~2.5 GB | ≈ W8A16 的 2 倍 | ppl +1~2% | **是（量产首选）** |
| **W4A16（AIMET/QAIRT · 逐层重建校准）** | INT4 权重 / FP16 激活 | ~2.5 GB | 与上相同 | ppl +1~3% | 是 |
...
- **座舱 Function Calling 量产**：**W4A16（经 AIMET/QAIRT 导出）**——decode 带宽减半、内存 ~2.5 GB 可承载，ppl 增幅对工具调用准确率影响很小。
- **精度优先、内存充裕**：W8A16。
- **开发调试 / 精度基准**：FP16。

...
**缺口定量化（per-demand 口径，让缺口可评审）**：遗留检测是 **per-demand（按次触发）**功能，实测漏检 **4.6%/demand（4.6×10⁻²/demand）**。对照任何 ASIL 等级下 per-demand 安全目标的常见量级（**示意**：约 10⁻³~10⁻⁶/demand，正式目标必须由 HARA 导出），现状**高出约 1~4 个数量级**——这不是「差一点」而是**结构性缺口**，只能靠独立冗余传感（毫米波/超声）+ 更高召回目标 + 多帧确认来闭合，无法靠论证或阈值微调接受。
...
ISO 26262 与 ISO 21448（SOTIF）成文早于 ML 组件的大规模上车，2024 年起有两项针对道路车辆 AI 安全的专项标准，量产安全论证应一并纳入评审范围：
...
- **ISO/TR 5469:2024（道路车辆 — AI 功能安全）**：给出 AI 系统的功能安全视图，为 AI 功能的危害分析、安全需求导出与验证提供框架，可作为 §4.2 HARA 的补充输入。
...
- **中国《汽车数据安全管理若干规定》(2021)**：车内处理原则、默认不收集原则、精度范围适用原则——DMS/OMS 舱内数据应端侧处理、非必要不上传云端。这是 §5.3「图不出车」边界的**直接法源**。
- **中国《数据安全法》(2021)**：数据分类分级、处理全生命周期安全管理、出境安全评估。
- **欧盟 GDPR**（若车型出海）：生物特征/健康相关推断、数据最小化、存储限制、数据主体权利。
- **EU AI Act (2024 生效)**（若车型出海）：AI 系统按风险分级监管，舱内 DMS/OMS 属安全相关 AI，需评估风险等级并建立数据治理与日志留存。
...
> **法规锚点**：本节设计对应 **UNECE R156（SUMS，软件更新管理体系）** 与 **UNECE R155（CSMS，网络安全管理体系）** 及 **ISO/SAE 21434** 道路车辆网络安全工程的落地要求——更新包签名、版本可追溯、支持回滚、更新通道纳入风险管理。法规框架总览见 [数据合规与评估篇 §1.2](../../general/data-pipeline.html)；数据飞轮中模型 OTA 的分级委派（LoRA 热更新 / Prompt 更新 / 模型整体更新的风险分级）见 [§3.4](../../general/data-pipeline.html)，本节是其「怎么安全地更新」的车端工程实现。
...
> **法规锚点**：本节设计对应 **UNECE R155（CSMS）** / **R156（SUMS）** 与 **ISO/SAE 21434** 在「谁可以更新 / 解密 / 加载软件」上的工程落地——信任链是其安全更新与完整性要求的车端实现。法规框架见 [数据合规与评估篇 §1.2](../../general/data-pipeline.html)，OTA 分级委派见 [§3.4](../../general/data-pipeline.html)。

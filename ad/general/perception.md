...
- **标注成本**：3D 占据 GT 昂贵——通常靠多帧 LiDAR 聚合 + 离线重建生成 (见 §10.5 的 4D 自动标注)，或借助仿真；SelfOcc 一类自监督方法试图绕开 GT。

> [!TIP]
> **"要不要上 Occupancy"的决策框架**
>
> 别把它当免费的安全加成，按三问决策：(1) **场景开放集风险高吗**——城区长尾障碍、越野/施工区收益大，高速结构化场景收益递减；(2) **算力/带宽预算够吗**——稠密占据的代价是立方级的，预算紧就先上稀疏/2D 化变体或降分辨率；(3) **GT 从哪来**——没有 4D 自动标注管线，占据的监督信号本身就是瓶颈（呼应 §10.5）。三问都成立才值得，否则先用检测 + 可行驶空间兜底。
...
- **数据生成/增强**：合成逼真但罕见的场景 (恶劣天气、事故、长尾目标)，缓解数据稀缺——对 corner case 尤其有价值，与 §10 数据飞轮直接打通（世界模型是数据飞轮的"生成引擎"）。
...
越野与非结构化场景（无车道线、无交通标志、开放集地形）是与结构化道路**完全不同的感知范式**——从「检测已知目标」变为「理解地形物理属性」（从外观推断含水量、承载力等）。因其自成体系，已拆分为独立专项文档：

> [!TIP]
> **越野/非结构化场景** → [越野与非结构化场景](../projects/offroad/offroad-scenarios.html)
>
> 覆盖：地形分类与可通行性分析（几何 + 语义双通道）、结构化道路 vs 越野核心差异、数据稀缺与前沿方向、端侧落地考量。与本篇 §3 占据预测（开放集兜底）、§4 4D 成像雷达（穿透烟尘/大雨）互为呼应。

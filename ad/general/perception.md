...
> **本篇讲什么**
>
> 自动驾驶感知与决策的算法主线，九部分（本篇是领域知识层篇幅最大的一篇，重点补齐了 Occupancy、4D 成像雷达、世界模型、Mapless 等通识缺口）：
>
...
| **SparseOcc / GaussianFormer-2** | 全稀疏 / 分层稀疏表示 | 把稀疏化推向极致：只表征表面附近的体素、或由粗到细分层放置再细化，进一步压算力与显存 (2024-2025) |
| **SelfOcc** | 自监督占据 | 不依赖昂贵占据 GT，用重建/一致性自监督，缓解标注瓶颈 |
| **OccWorld** | 占据世界模型 | 同时预测未来占据 + 自车位姿，把占据从"感知"推向"预测" |

UniAD 的 OccFormer (见 §6.1) 同样把占据作为中间表征喂给规划。另一条延伸是 **3D 占据 → 4D 占据 (占据 + flow 预测)**：不只回答"现在哪里被占据"，还要回答"未来哪里将被占据"，直接服务规划 (如 Cam4DOcc 一类基准)——与 OccWorld 一样，都是"从感知走向预测"的同一方向。
...
- **挑战**：点云仍比 LiDAR 稀疏、噪声大，存在多径反射 (护栏、隧道)；标注数据集稀缺；如何在 BEV 空间与相机/LiDAR 高效融合是活跃研究方向 (如 radar-camera 融合的 4D 检测：RCBEVDet、RadarBEVNet 等)。
...
- **2025-2026 定位**：研究热点从"能不能检出目标"转向"radar 与相机在 BEV/占据空间的高效融合、多径鬼影抑制、恶劣天气下的鲁棒性量化"；量产侧已有部分国产新能源车型将 4D 雷达上车（多作为去 LiDAR 的降本/冗余配置），但整体生态成熟度（标注、基准、融合范式）仍明显落后于相机/LiDAR。
...
>
> 因此**预测/规划的严肃评测要走闭环仿真**（nuPlan、CARLA Leaderboard、Bench2Drive、Waymax、NAVSIM 一类），让模型的动作真正改变后续场景。这个缺陷在端到端规划上被放大成"刷分陷阱"，详见 §6.3 的 WARNING。
...
- **token 化 / 自回归生成 (2024-2025)**：MotionLM、SMART 等把多智能体轨迹离散化为 token，用 next-token prediction 自回归生成，曾拿下 Waymo Open Motion Dataset 交互式赛道榜首——轨迹预测与大语言模型在技术路线上开始合流，也天然便于与 VLM/世界模型拼接。

### 5.3 与端到端的融合

在模块化方案里，预测是独立模块；在端到端方案 (如 UniAD 的 MotionFormer、VAD) 里，预测被并入统一模型，作为中间任务与检测/建图/规划共享特征、梯度端到端传播。好处是信息不被人为割裂，挑战是预测误差会直接传导到规划。

到 2025-2026，"独立轨迹预测模块"在量产与研究里都在收缩：预测能力被**吸收进端到端模型**（作为中间任务）与**世界模型**（作为场景推演），评测重心也从开环 ADE/FDE 转向闭环/仿真口径（呼应 §5.1 WARNING）。预测没有消失，而是从"交付物"变成了"中间表征"。
...
> - **正确做法**：(1) 报告**去掉 ego status 输入**后的性能；(2) 走**闭环/仿真评测**（nuPlan、CARLA Leaderboard、Bench2Drive、Waymax；NAVSIM 属基于真实回放的**非反应式**评测，但用 PDM 规则化指标打分，堵住了"纯 L2 刷分"的口子），让自车动作真正影响后续场景；(3) 关注**碰撞率/越界率/舒适度**等结果性指标，而非单纯轨迹距离。
>
...
- **RL 后训练与"慢思考" (2025-)**：借鉴大语言模型的配方，用**强化学习 + 思维链推理**训练驾驶 VLM/VLA（如 AlphaDrive 一类工作），提升复杂路口博弈与长尾场景决策；这是 2025-2026 增长最快的研究方向之一，但推理时延与算力的车端落地仍在早期——"慢思考"目前只能放在低频系统或云端。
- **世界模型驱动的端到端**：用世界模型（见 §7）做**闭环数据生成与闭环评测**，甚至直接在"想象"中做规划（imagination-based planning）。这是把 §6.3 CAUTION 里"开环刷分"问题从根上解决的方向，也是 2025-2026 投入最重的方向之一；2025 年起已有量产玩家把世界模型写进官方架构（见下表华为 WEWA）。
- **一段式端到端的量产收敛**：从"学术 demo"变成"多家量产车的默认架构"，但安全兜底层始终保留（见下表）。

| 玩家 | 公开形态（截至 2026） | 路线归属 |
| :--- | :--- | :--- |
| **Tesla FSD v12→v14** | photon-to-control，神经网络取代大量手写规划代码；v13/v14 (2025-2026) 持续扩大模型规模与上下文 | 真·一段式端到端 |
| **华为 ADS 3.0/4.0** | ADS 3.0 (2024)：GOD 通用障碍网络 + PDP 预测决策规划；ADS 4.0 (2025)：WEWA 架构——云端 World Engine 世界引擎生成难例场景 + 车端 World Action Model 世界行为模型 | 两段式端到端 → 世界模型驱动 |
| **小鹏 XNGP** | 端到端大模型量产 (2024) → 车端 VLA + 云端世界基础模型做训练/评测 (2025) | 两段式 → 一段式/VLA |
| **理想 AD Max** | 端到端 + VLM 双系统 (2024) → VLA 驾驶大模型 (2025) | 双系统 → VLA |
| **Waymo Driver** | 模块化感知 + ML 规划为主，EMMA 等为研究探索 | 模块化为主 |
| **Wayve** | LINGO / GAIA 系列，语言 + 世界模型驱动 | 端到端 + 世界模型 |

2025 年起更多量产玩家（蔚来及其 NWM 世界模型、小米 HAD、Momenta 一段式方案等）也先后发布端到端/世界模型/VLA 形态；上表仅为代表性举例，各家版本迭代很快，具体形态以官方最新发布为准。
...
| 工作 | 核心 |
| :--- | :--- |
| **GAIA-1 (2023) / GAIA-2 (2025, Wayve)** | 生成式驾驶世界模型，可用文本/动作条件生成未来驾驶视频；GAIA-2 支持多相机视角、agent/动作可控的场景生成，面向数据生成与闭环评测 |
...
| **NVIDIA Cosmos (2025)** | 世界基础模型平台 (Predict / Transfer / Reason)，模型权重开源发布；Cosmos Reason 提供物理常识推理，面向物理 AI 的生成式仿真与数据引擎 |
| **Genie 系列 (DeepMind)** | 通用可交互世界模型：Genie / Genie 2 (2024) → **Genie 3 (2025)**，从单图/文本提示生成**实时可操作**环境（Genie 3 支持 720p/24fps 交互与分钟级场景一致性），是"世界模型 = 可交互仿真器"路线的代表（通用世界模型，非驾驶专用） |
...
> - **工具链化（进入量产管线）**：云端世界模型已成为头部玩家数据/评测管线的标配——华为 ADS 4.0 的 World Engine（云端生成难例场景）、蔚来 NWM、小鹏世界基础模型、Wayve GAIA-2 等；车端"生成式规划"仍受算力约束，现阶段的主要形态是**云端生成 + 车端判别**。
>
...
- **P-MapNet 一类 (2024-)**：把道路级 SD 地图作为额外先验注入在线矢量建图——这正是 §8.3 "三层组合"思路在方法层的实现：不追求"纯无图"，而是用最便宜的先验换稳定性。
...
> **飞轮转得快 ≠ 转得好：三道质量门控**
>
> 数据飞轮的真实风险不是"转不起来"，而是**坏数据/坏模型在闭环里被规模化放大**。成熟管线会在三处设门控：
>
> - **数据门控**：入库前做嵌入级去重（海量相似高速场景会淹没长尾）、分布均衡（按场景/天气/地域配额）、标签噪声率阈值（呼应 §9.5 的质量校验）——车队规模下 garbage in 就是 garbage out 的放大器。
> - **回归门控**：新模型必须同时满足"**旧场景不退化 + 目标场景有提升**"，回归集与目标集分开评测。"修好一个 corner case、引入三个新回归"是迭代中最常见的负飞轮，只盯目标场景指标的管线必然踩坑。
> - **发布门控**：影子评估 (§9.2) → 小流量灰度 → 分区域放量，每一级都有**预先设定**的放量/回滚判据（接管率、AEB 误触发率等），而不是"看起来不错就全量"。
>
> 门控指标本身也要防"刷分"：接管率可以通过收窄 ODD 变好看，必须与覆盖范围指标一起看。
...
**硬件事件触发**：急刹车 (如减速度 > 0.4g，示例阈值，按车型/法规标定)、急转向、ABS/AEB 触发、人类接管等事件自动标记为 corner case 候选。
...
- **多传感器聚合**：把多帧 LiDAR 点云 + 多相机图像聚合，通过离线重建 (点云聚合 / NeRF / 3DGS——StreetGaussians、DrivingGaussian 一类**动态场景重建**在 2024-2025 成为自动标注的标准底座) 得到一致的 4D 场景。

典型流程：**离线感知 (大模型) → 多目标跟踪 → 4D 场景重建 → 标签反向传播/传递 → 质量校验**。4D 重建保证标签在多帧间时序一致 (同一目标的框跨帧平滑)，并能生成占据等稠密 GT (呼应 §3)。这是支撑大规模数据闭环的核心引擎。

**2025-2026 的进展**集中在两处：(1) **重建一致性**——动静分离、4D 高斯式重建让长序列标签漂移更小；(2) **语义级质检**——用 VLM 辅助做标签的语义校验与长尾属性标注（"这个框住的是掉落货物还是阴影"），把人工抽检从"逐框看"变成"看 VLM 标出的可疑项"。
